...
function mapSubmissionSlim(row) {
  if (!row) return null;
  return {
    id: row.id,
    taskId: row.task_id,
...
function mapNotification(row) {
  return {
    ...
    earnerName: row.earner_name || void 0,
    taskTitle: row.task_title || void 0,
    submittedAt: row.submitted_at instanceof Date ? row.submitted_at.toISOString() : row.submitted_at || (row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at),
    reviewUrl: row.review_url || void 0,
    createdAt: row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at,
...
var FOLLOW_PROTECTION_BATCH_SIZE = 25;
function isFollowProtectedTask(category) {
...
app.post("/api/earner/tasks/:id/submit", async (req, res) => {
  ...
  try {
    ...
    if (concurrentSub.rows.length > 0) {
      await client.query("ROLLBACK");
      return res.status(400).json({
        error: concurrentSub.rows[0].status === "Pending" /* PENDING */ ? "Your submission is already pending review. Please wait for the advertiser's decision." : "You have already successfully completed this task."
      });
    }
    ...
    if (isFollowProtectedTask(task.category)) {
      const batchCap = Math.min(
        Math.floor(task.filledSlots / FOLLOW_PROTECTION_BATCH_SIZE) * FOLLOW_PROTECTION_BATCH_SIZE + FOLLOW_PROTECTION_BATCH_SIZE,
        task.totalSlots
      );
...
app.delete("/api/earner/submissions/:id", async (req, res) => {
  try {
    ...
    try {
      ...
      if (submission.status !== "Pending" /* PENDING */ && submission.status !== "Rejected" /* REJECTED */) {
        await client.query("ROLLBACK");
...
app.post("/api/advertiser/tasks", async (req, res) => {
  try {
    ...
    if (totalSlots === void 0 || totalSlots === null || totalSlots === "") {
    ...
    if (isNaN(slots) || slots < 1 || slots > 1e6) {
...
app.post("/api/advertiser/submissions/:id/review", async (req, res) => {
  try {
    ...
    if (commissionData) {
      void pool.query("SELECT COALESCE(SUM(amount),0) AS total FROM admin_commissions WHERE type='task_commission'").then((totalRes) => console.log(
...
async function sendBrowserPushToAdmins(payloadJson) {
...
async function notifyAdmin(notification) {
  ...
  adminClients.forEach((client) => {
    if (client.readyState === import_ws.WebSocket.OPEN) client.send(wsPayload);
  });
  ...
  const pushTitle = notification.type === "submission" ? "\u{1F4E5} New Task Submission" : notification.type === "withdrawal" ? "\u{1F4B8} Withdrawal Request" : "\u{1F4B3} New Deposit";
  const pushBody = notification.type === "submission" ? `${notification.earnerName || "An earner"} submitted "${notification.taskTitle || "a task"}" for review \xB7 ${timeStr}` : notification.message;
  const pushUrl = notification.reviewUrl || (notification.type === "submission" ? "/admin/audits" : notification.type === "withdrawal" ? "/admin/withdrawals" : "/admin/stats");
  const pushPayload = JSON.stringify({
    title: pushTitle,
    body: pushBody,
    icon: "/icon-192.png",
  ...
  sendBrowserPushToAdmins(pushPayload).then((sent) => {
